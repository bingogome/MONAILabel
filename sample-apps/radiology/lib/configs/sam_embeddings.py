...
class SAMEmbeddings:
            ...
            for name in tqdm(all_lab_imgs):
                ...
                if np.sum(gt_data) > 1000:
                    ...
                    for view, indexes in zip(['axial', 'sagittal', 'coronal'], [axial_index, sag_index, cor_index]):
                        ...
                        imgs, gts, img_embeddings = self._preprocess_img(self, image_data_pre, idx_min, idx_max, view, gt_data, self.sam_model, self.device)
    ...
    @staticmethod
    def _preprocess_img(self, image_data_pre, idx_min, idx_max, view, gt_data, sam_model, device):
        imgs = []
        gts = []
        img_embeddings = []
    ...
    def _save_npz(imgs, gts, img_embeddings, embed_path, name, view, label_id):
        npz_path = os.path.join(embed_path, name + f'_{view}_view_label_id_{label_id}.npz')
        # save to npz file
        if len(imgs) > 1:
            ...
            img_idx[bd, :] = [255, 0, 0]
            io.imsave(os.path.join(embed_path, name + f'_{view}_view_label_id_{label_id}.png'), img_idx, check_contrast=False)
